...
public class BOLOAlertDAO extends BaseDAO<BOLOAlert> {
    ...
    public BOLOAlert findById(int id) throws SQLException {
        String sql = "SELECT * FROM vw_active_bolo_alerts WHERE id = ?";
    ...
    public List<BOLOAlert> findByPriority(String priority) throws SQLException {
        String sql = "SELECT * FROM vw_active_bolo_alerts WHERE priority = ? ORDER BY alert_date DESC";
        return executeQuery(sql, priority);
    }

    @Override
    public boolean insert(BOLOAlert entity) throws SQLException {
        String sql = "CALL sp_generate_bolo_alert(?, ?, ?, ?)";
    ...
    public boolean cancelAlert(int alertId) throws SQLException {
        String sql = "UPDATE bolo_alerts SET status = 'CANCELLED', updated_at = CURRENT_TIMESTAMP WHERE id = ?";
    ...
    public boolean update(BOLOAlert entity) throws SQLException {
        String sql = "UPDATE bolo_alerts SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
